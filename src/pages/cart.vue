...
export default {
  ...
  methods: {
    ...
    //购物车下单
    order() {
      //返回bool(回调函数都返回true返回true)
      let isCheck = this.list.every((item) => !item.productSelected);
      if (isCheck) {
        Message.warning("下单至少选择一件商品！");
      } else {
    ...
    delProduct(item) {
      ...
      instance.delete(`/carts/${item.productId}`, { id }).then((res) => {
    ...
    renderData(res) {
      ...
      this.checedkNum = this.list.filter((item) => item.productSelected).length;
    },
  },
};
</script>
  ...
  .wrapper {
    ...
    .cart-box {
      ...
      .checkbox {
        ...
        cursor: pointer;
        &.checked {
          background: url("./../assets/imgs/icon-gou.png") #ff6600 no-repeat
            center;
